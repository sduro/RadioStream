...
#update: http://fmstream.org/index.php?c=E
...
def main():
    ...
    """
    #buttons_init() #Descomentar para Raspi
    #lcd_init() #Descomentar para Raspi
    #lcd_print("Iniciando", "Emisora") #Descomentar para Raspi   
    tree = ET.parse('emisoras.xml')
    root = tree.getroot()
    r = reproductor(1,len(root))     
    r.play()    
    option = {1:r.play,2:r.prev,3:r.next,4:r.stop,5:r.exit}
    try:
        while True:
            num=int(input())     #comentar para la version raspi / Descomentar esta linea para la version x86
